...
AC_DEFUN([LDISKFS_AC_EXT_SOURCES], [
	AS_IF([test x$BACKFS = xext4], [
		LDISKFS_AC_EXT4_SOURCES
	], [test x$BACKFS = xext3], [
		LDISKFS_AC_EXT3_SOURCES
	], [
		AC_MSG_ERROR([*** \$BACKFS is neither ext3 nor ext4])
	])
])
...
AC_DEFUN([LDISKFS_AC_LDISKFS_SERIES], [
	...
	AS_CASE([${LINUX_VERSION}],
		...
		[2.6.32-2*rh6*], [LDISKFS_SERIES="2.6-rhel6.series"],
		[2.6.32-2*ch5*], [LDISKFS_SERIES="2.6-rhel6.series"],
		[2.6.32-3*rh6*], [LDISKFS_SERIES="2.6-rhel6.4.series"],
		[2.6.32-3*ch5*], [LDISKFS_SERIES="2.6-rhel6.4.series"],
